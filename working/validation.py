...
def EvaluateSubmission(predictions, input_dir, scaling_dict, thresholds_q, thresholds_t, output_dir=None):
    ...
    if output_dir is not None:
        ...
        maa_result = np.mean(list(maa_per_scene.values()))
        df = pd.DataFrame({"scene":scene_list, "pair":pair_list, "maa":maa_list})
        output_csv = os.path.join(output_dir, f"validation_all_mAA={maa_result}.csv")
        df.to_csv(output_csv, index=False)

    return np.mean(list(maa_per_scene.values())), maa_per_scene, errors_dict_q, errors_dict_t

def load_scaling_dict(scaling_factors_path):
    scaling_dict = {}
...
def evaluate(input_dir, sample_id_list, fund_matrix_list, output_dir=None):
    thresholds_q = np.linspace(1, 10, 10)
    thresholds_t = np.geomspace(0.2, 5, 10)

    # Load per-scene scaling factors.
    scaling_dict = load_scaling_dict(os.path.join(input_dir, f"scaling_factors.csv"))
    ...
    maa, maa_per_scene, errors_dict_q, _ = EvaluateSubmission(
        predictions,
        input_dir,
        scaling_dict,
        thresholds_q,
    ...
    return maa, maa_per_scene, errors_dict_q
...
def main(cfg: DictConfig):
    ...
    for scene in scene_list:
        output_csv = os.path.join(output_dir, f"train_pred_cov{covisibility_thr_min}-{covisibility_thr_max}_max_num_pairs{cfg.max_num_pairs}_{scene}.csv")
        if os.path.exists(output_csv):
            df_scene = pd.read_csv(output_csv)
            df_all = pd.concat([df_all, df_scene])
    sample_id_list = df_all["sample_id"].values
    fundamental_matrix_list = df_all["fundamental_matrix"].values

    # Evaluate the predictions.
    maa, maa_per_scene, errors_dict_q = evaluate(
        input_dir,
        sample_id_list, fundamental_matrix_list,
        output_dir)
    ...
    for scene, cur_maa in maa_per_scene.items():
        ...
        count += len(errors_dict_q[scene])
    print()
    print(f'Full dataset: mAA={maa:.05f} (n={count})')
    print()
